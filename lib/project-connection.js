...
function attachConnection(ctx) {
  ...
  var allowAutoMode = ctx.allowAutoMode !== false;  // windmill: allowAutoMode
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    sendTo(ws, { type: "info", cwd: cwd, slug: slug, project: title || project, version: currentVersion, debug: !!debug, dangerouslySkipPermissions: dangerouslySkipPermissions, allowAutoMode: allowAutoMode, osUsers: osUsers, lanHost: lanHost, projectCount: _filteredProjects.length, projects: _filteredProjects, projectOwnerId: projectOwnerId, ownerLocked: ownerLocked });  // windmill: allowAutoMode

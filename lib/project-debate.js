...
function attachDebate(ctx) {
  ...
  function handleDebateSkillSetup(ws, session, debate, msg, mateCtx, moderatorProfile, briefPath) {
    ...
    var _debateUserMsg = { type: "user_message", text: craftingPrompt, ts: Date.now() };
    setupSession.history.push(_debateUserMsg);
    ctx.sm.appendToSessionFile(setupSession, _debateUserMsg);
    ctx.sendToSession(setupSession.localId, _debateUserMsg);

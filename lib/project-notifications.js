...
function attachNotifications(ctx) {
  ...
  function notify(event, data) {
    data = data || {};
    // clay local patch (P3.T5): drop the per-step tui_attention firehose at the
    // server choke point — clay's settings-UI toggles are client-only and do NOT
    // gate the bell. Keeps useful types (auth_required, loop_complete, mate_dm).
    if (event === "tui_attention") return null;

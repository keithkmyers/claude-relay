    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (xmTarget && (xmTarget.vendor === "claude" || !xmTarget.vendor)) {
          ...
          if (xmTarget.mode === "tui") {
            if (xmPref === "gui") {
              // Born-TUI under GUI pref: hydrate history + drop the PTY so it
              // renders via the SDK chat (continue-via-GUI). session.mode
              // stays 'tui' so flipping the pref back restores the terminal.
              prepareTuiSessionForGuiView(xmTarget);
              xmTarget.runtimeMode = "gui";
              xmTarget.runtimeTerminalId = null;
              xmTarget.tuiSuspended = false;
            } else if (tm && typeof xmTarget.terminalId === "number" && tm.has(xmTarget.terminalId)) {
              // Native PTY still alive (live session, e.g. just created or
              // switched away from this run) - reattach directly, no respawn.
              xmTarget.runtimeMode = "tui";
              xmTarget.runtimeTerminalId = xmTarget.terminalId;
              xmTarget.tuiSuspended = false;
            } else if (tm && typeof xmTarget.runtimeTerminalId === "number" && tm.has(xmTarget.runtimeTerminalId)) {
              // A resume PTY from earlier this run is still alive - reattach.
              xmTarget.runtimeMode = "tui";
              xmTarget.tuiSuspended = false;
            } else {
              // No live PTY (typical after a daemon restart, or claude exited).
              // Lazy resume: show the transcript read-only and let the user
              // click Resume to spawn `claude --resume`. Spawning on every
              // click would pile up idle claude processes - the whole point of
              // this view. The PTY is created in resume_tui_session.
              prepareTuiSessionForGuiView(xmTarget);
              xmTarget.runtimeMode = null;
              xmTarget.runtimeTerminalId = null;
              xmTarget.tuiSuspended = true;
            }
          } else {
            // Born-GUI session. Under TUI pref, spawn a transient
            // `claude --resume` PTY; otherwise render as GUI chat.
            var xmRuntime = computeRuntimeMode(xmTarget, xmPref);
            if (xmRuntime === "tui" && xmTarget.cliSessionId) {

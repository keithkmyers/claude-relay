...
var systemQuery = require("./system-query");
...
function createProjectContext(opts) {
  ...
  var sdk = createSDKBridge({
    ...
    onSessionNamingTrigger: function (session) {
      var consent = getAutoNamingSetting(session);
      console.log("[naming-trigger] session=" + session.localId + " consent=" + consent + " firstMsgLen=" + (session._firstMessageText || "").length);
      if (consent === "off") {
        console.log("[naming-trigger] consent=off, skipping");
        return;
      }
      systemQuery.run("session-naming", {
        consent: consent,
        firstMessage: session._firstMessageText || "",
        onResult: function (name, meta) {
          console.log("[naming-trigger] onResult fired: name=" + JSON.stringify(name) + " meta=" + JSON.stringify(meta) + " session=" + session.localId);
          if (!name || typeof name !== "string" || name.length < 2) {
            console.log("[naming-trigger] REJECTED: invalid name");
            return;
          }
          var cleanName = name.substring(0, 60);
          if (meta.consent === "auto") {
            console.log("[naming-trigger] AUTO-APPLYING: " + cleanName + " to session " + session.localId);
            session.title = cleanName;
            sm.saveSessionFile(session);
            sm.broadcastSessionList();
            sendToSession(session.localId, { type: "session_renamed", id: session.localId, title: cleanName, fromAI: !!meta.fromAI });
            if (session.cliSessionId) {
              getSDK().then(function(sdkMod) {
                sdkMod.renameSession(session.cliSessionId, cleanName, { dir: cwd }).catch(function() {});
              }).catch(function() {});
            }
          } else {
            console.log("[naming-trigger] SUGGESTING: " + cleanName + " to session " + session.localId);
            sendToSession(session.localId, {
              type: "session_name_suggestion",
              id: session.localId,
  ...
  // --- AI session naming helper ---
  function getAutoNamingSetting(session) {
    if (usersModule.isMultiUser() && session && session.ownerId) {
      return usersModule.getAutoNaming(session.ownerId);
    }
    if (typeof opts.onGetDaemonConfig === "function") {
      var dc = opts.onGetDaemonConfig();
      return dc.autoNaming || "off";
    }
    return "off";
  }

  // --- System-initiated query setup ---
  systemQuery.init({
    getSDK: getSDK,
    cwd: cwd,
  ...
  // Heuristic fallback: extract a clean 3-5 word title from the first message
  function heuristicSessionName(context) {
    var text = (context.firstMessage || "").trim();
    if (!text) return "New Session";
    // Strip common filler prefixes
    text = text.replace(/^(hey|hi|hello|please|can you|could you|i'd like to|i want to|i need to|help me|let's)\s+/i, "");
    // Take first 5 meaningful words
    var words = text.split(/\s+/).filter(function (w) { return w.length > 1; });
    var title = words.slice(0, 5).join(" ");
    if (title.length > 50) title = title.substring(0, 50);
    // Capitalize first letter
    if (title) title = title.charAt(0).toUpperCase() + title.substring(1);
    return title || "New Session";
  }

  systemQuery.register({
    id: "session-naming",
    model: "sonnet",
    maxTokens: 30,
    rateLimit: { max: 30, windowMs: 3600000 },
    fallback: heuristicSessionName,
    buildPrompt: function (context) {
      return "Generate a concise Title Case label (3-5 words) that describes what this chat session is about. " +
        "Think of it like a chat tab name — specific enough to recognize later. " +
        "Reply with ONLY the title. No quotes, no punctuation, no prefix like 'Title:'.\n\n" +
        "Examples:\n" +
        "  'can you help me refactor the auth module' -> Refactor Auth Module\n" +
        "  'whats the best way to handle errors in rust' -> Rust Error Handling\n" +
        "  'debugging this weird npm install failure' -> NPM Install Debug\n\n" +
        "User message:\n" + (context.firstMessage || "").substring(0, 500);
    },
    parseResponse: function (text) {
      // Strip quotes, backticks, trailing punctuation, common prefixes
      var name = text.trim()
        .replace(/^["'`]+|["'`]+$/g, "")
        .replace(/^(title|label|name)\s*:\s*/i, "")
        .replace(/[.!?,;:]+$/, "")
        .trim();
      // Enforce 3-5 word limit loosely (cap at 7)
      var words = name.split(/\s+/);
      if (words.length > 7) name = words.slice(0, 5).join(" ");
      // Ensure Title Case on first word at minimum
  ...
  function runNextIteration() {
    ...
    var userMsg = { type: "user_message", text: loopState.promptText, ts: Date.now() };
  ...
  function runJudge() {
    ...
    var userMsg = { type: "user_message", text: judgePrompt, ts: Date.now() };
  ...
  function handleConnection(ws, wsUser) {
    ...
    if (active && !autoCreated) {
      ...
      var quickFrom = sm.findFromIndexForTurns(active.history, sm.HISTORY_QUICK_TURNS);

      // Send only the last HISTORY_QUICK_TURNS turns on open.  No auto-stage-2
      // backfill — older history loads only when the user clicks the "Load
      // earlier messages" button (handled via the load_more_history handler).
      // history_meta.from MUST match what we actually delivered (quickFrom),
      // otherwise the client's historyFrom marker is wrong and the sentinel
      // button is mis-hidden / mis-shown.
      sendTo(ws, { type: "history_meta", total: total, from: quickFrom });
      var _hitems = [];
      for (var i = quickFrom; i < total; i++) {
        _hitems.push(hydrateImageRefs(active.history[i]));
      }
      sendTo(ws, { type: "history_batch", items: _hitems });
  ...
  function scheduleMessage(session, text, resetsAt) {
    ...
    session.scheduledMessage = {
      ...
      timer: setTimeout(function () {
        ...
        var schedUserMsg = { type: "user_message", text: text, ts: Date.now() };
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "load_more_history") {
      ...
      if (typeof msg.target === "number") {
        // Explicit target index (e.g. in-session search wanting a specific range)
        from = sm.findTurnBoundary(session.history, Math.max(0, msg.target));
      } else {
        // Default: paginate by turns.  Grabs HISTORY_LOAD_MORE_TURNS complete
        // user↔assistant turns ending just before `before`.  Item count varies
        // with content (chat turns are tiny, coding turns with tools are big),
        // but the user-visible chunk size stays predictable.
        from = sm.findFromIndexForTurns(session.history, sm.HISTORY_LOAD_MORE_TURNS, before);
      }
      var to = before;
    ...
    if (msg.type === "set_session_status") {
      if (typeof msg.sessionId === "number") {
        ...
        sm.setSessionStatus(msg.sessionId, newStatus);
      }
      return;
    }
    ...
    if (msg.type === "accept_session_name") {
    ...
    if (msg.type === "rename_session") {
      if (msg.id && sm.sessions.has(msg.id) && msg.title) {
        var s = sm.sessions.get(msg.id);
        s.title = String(msg.title).substring(0, 100);
        s.aiNamingDone = true;  // manual rename — don't suggest again
        sm.saveSessionFile(s);
        sm.broadcastSessionList();
    ...
    if (msg.type === "permission_response") {
      ...
      if (decision === "allow_clear_context") {
        ...
        Promise.race([
          ...
          try {
            ...
            var userMsg = { type: "user_message", text: planPrompt, planContent: clientPlanContent || null, ts: Date.now() };
      ...
      if (decision === "deny_with_feedback") {
        ...
        if (feedback) {
          setTimeout(function () {
            var userMsg = { type: "user_message", text: feedback, ts: Date.now() };
            session.history.push(userMsg);
            sm.appendToSessionFile(session, userMsg);
    ...
    if (msg.type === "ralph_wizard_complete") {
      ...
      if (wData.mode === "own" && wData.promptText) {
        ...
        if (wData.judgeText) {
          ...
          var _judgeCraftUserMsg = { type: "user_message", text: judgeCraftPrompt, ts: Date.now() };
          judgeCraftSession.history.push(_judgeCraftUserMsg);
          sm.appendToSessionFile(judgeCraftSession, _judgeCraftUserMsg);
          sendToSession(judgeCraftSession.localId, _judgeCraftUserMsg);
      ...
      var _craftUserMsg = { type: "user_message", text: craftingPrompt, ts: Date.now() };
      craftingSession.history.push(_craftUserMsg);
      sm.appendToSessionFile(craftingSession, _craftUserMsg);
      sendToSession(craftingSession.localId, _craftUserMsg);
    ...
    var userMsg = { type: "user_message", text: msg.text || "", ts: Date.now() };
    ...
    if (!session.title) {
      ...
      // Cache first message text for AI session naming (before it gets modified)
      if (!session.aiNamingDone) {
        session._firstMessageText = (msg.text || "").substring(0, 500);
      }
      sm.saveSessionFile(session);
      sm.broadcastSessionList();

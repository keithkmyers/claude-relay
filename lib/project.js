...
function createProjectContext(opts) {
  ...
  var sdk = createSDKBridge({
    ...
    onTurnDone: isMate ? function (session) { digestDmTurn(session); } : null,
  });
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "memory_list") {
      ...
      // Return newest first
      entries.reverse();
      sendTo(ws, { type: "memory_list", entries: entries });
      return;
    }
  ...
  // Digest DM turn for mate projects - creates a temporary session to extract memory
  var _dmDigestPending = false;
  function digestDmTurn(session) {
    if (!isMate || _dmDigestPending) return;
    var mateId = path.basename(cwd);
    var mateCtx = matesModule.buildMateCtx(projectOwnerId);
    if (!matesModule.isMate(mateCtx, mateId)) return;

    // Extract last user message and response from history
    var lastUserText = "";
    var lastResponseText = "";
    for (var hi = session.history.length - 1; hi >= 0; hi--) {
      var entry = session.history[hi];
      if (!lastResponseText && entry.type === "delta" && entry.text) {
        lastResponseText = entry.text;
      }
      if (!lastUserText && entry.type === "user_message" && entry.text) {
    ...
    var mateDir = matesModule.getMateDir(mateCtx, mateId);
    var knowledgeDir = path.join(mateDir, "knowledge");
    var claudeMd = loadMateClaudeMd(mateCtx, mateId);

    var digestContext = [
      "You just had a DM conversation with the user.",
      "User said: " + lastUserText,
      "You responded: " + (lastResponseText.length > 1000 ? lastResponseText.substring(0, 1000) + "..." : lastResponseText),
    ].join("\n");

    var digestPrompt = [
      "[SYSTEM: Session Digest Request]",
      "The conversation turn has ended. Summarize this DM exchange from YOUR perspective for your long-term memory.",
      "Output ONLY a single valid JSON object (no markdown, no code fences, no extra text).",
      "Schema:",
      "{",
      '  "date": "YYYY-MM-DD",',
      ...
      '  "topic": "short topic description",',
      '  "my_position": "what I said/recommended",',
      '  "user_intent": "what the user wanted or was asking about",',
      '  "decisions": "what was decided, or null if pending",',
      '  "open_items": "what remains unresolved",',
      '  "user_sentiment": "how the user seemed to feel about this topic"',
      "}",

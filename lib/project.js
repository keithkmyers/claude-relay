...
function createProjectContext(opts) {
  ...
  function gateMemory(mateCtx, mateId, conversationContent, callback) {
    var mateDir = matesModule.getMateDir(mateCtx, mateId);
    var knowledgeDir = path.join(mateDir, "knowledge");
    ...
    sdk.createMentionSession({
      claudeMd: "",
      model: "haiku",
      initialContext: gateContext.join("\n"),
      ...
      onError: function (err) {
        ...
        if (_gateSession) try { _gateSession.close(); } catch (e) {}
        // On error, skip memory creation
        callback(false);
      },
    }).then(function (gs) {
      _gateSession = gs;
      if (!gs) callback(false);
    }).catch(function (err) {
  ...
  function digestDebateParticipant(session, mateId, debate, role) {
    ...
    var debateContent = "Debate topic: " + debate.topic + "\nRole: " + role;
    if (debate.history && debate.history.length > 0) {
      var historySnippets = [];
      for (var hi = 0; hi < debate.history.length && hi < 10; hi++) {
        var h = debate.history[hi];
        historySnippets.push((h.mateName || h.speaker || "Unknown") + ": " + (h.text || "").slice(0, 200));
      }
      debateContent += "\nDebate exchanges:\n" + historySnippets.join("\n");
    }

    // Gate check: ask Haiku if this is worth remembering
    gateMemory(debate.mateCtx, mateId, debateContent, function (shouldRemember) {
      if (!shouldRemember) {
        console.log("[debate-digest] Gate declined memory for debate, mate " + mateId);

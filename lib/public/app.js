...
import { initServerLogo, getServerFaviconUrl } from './modules/server-logo.js';
...
// --- DOM refs ---
  ...
  var headerNameSuggestionEl = $("header-name-suggestion");
  ...
  var replayBuffer = null; // array when buffering history for batch replay
  var historyRenderAbort = null; // cancellation token for async chunked history render
  var prependLiveBuffer = null; // queue for live WS messages arriving during chunked prepend render
  var processingHistoryChunk = false; // true only inside the sync for-loop of a chunk (distinguishes history items from live ws messages)
  var pendingAssistantTs = 0; // timestamp for the next assistant block (set by assistant_ts event)

  // --- Timestamp formatting helpers ---
  function _formatClockTime(ts) {
    var d = new Date(ts);
  ...
  function _formatMsgTime(ts) {
    return _formatClockTime(ts) + " \u00b7 " + _formatRelativeTime(ts);
  }

  // Periodically refresh all visible relative timestamps so they stay accurate
  setInterval(function () {
    var els = document.querySelectorAll(".dm-bubble-time[data-ts]");
    for (var i = 0; i < els.length; i++) {
      var t = parseInt(els[i].dataset.ts, 10);
      if (t) els[i].textContent = _formatMsgTime(t);
    }
  }, 30000);
  ...
  // Reset favicon cache when theme changes — re-apply server logo if set
  onThemeChange(function () {
    faviconOrigHref = null;
    var customLogo = getServerFaviconUrl();
    if (customLogo && customLogo !== 'favicon-banded.png') {
      setTimeout(function () {
        var fl = document.querySelector('link[rel="icon"]');
        if (fl) fl.href = customLogo;
      }, 50);
    }
  });
  ...
  // Listen for server logo changes to update favicon restore target
  window.addEventListener('server-logo-changed', function (e) {
    var logoUrl = e.detail && e.detail.url;
    if (logoUrl) {
      // When urgent blink stops, restore to the custom logo, not default
      if (faviconOrigHref) faviconOrigHref = logoUrl;
    } else {
      if (faviconOrigHref) faviconOrigHref = basePath + "favicon-banded.png";
    }
  });
  ...
  // After an initial history replay, async content (mermaid SVGs, images,
  // code-highlight markup, icon refresh, font metrics) inflates scrollHeight
  // over the next several hundred ms.  The single scrollToBottom() fired in
  // history_done lands on a pre-expansion layout, so those late reflows push
  // the user mid-conversation while content appears to "fill in below".  This
  // keeps the scroller pinned to the bottom until layout settles, or until
  // the user actively scrolls, whichever comes first.
  var postReplayStickyTimer = null;
  var postReplayObserver = null;
  function stickToBottomDuringReplayGrace(durationMs) {
    cancelPostReplaySticky();
    if (prependAnchor) return;

    var graceMs = durationMs || 1200;
    var cancelled = false;
    var startH = messagesEl.scrollHeight;
    ...
    // One snap immediately to align after the sync drain.
    snap();

    // ResizeObserver catches late layout expansion from async mermaid / image
    // decode / font load / highlight.js markup — snap back each time the
    // scroller's inner height grows during the grace window.
    if (typeof ResizeObserver !== "undefined") {
      postReplayObserver = new ResizeObserver(function () {
        if (cancelled) return;
        if (messagesEl.scrollHeight !== startH) {
          startH = messagesEl.scrollHeight;
          snap();
        }
      });
      try { postReplayObserver.observe(messagesEl); } catch (e) {}
    }

    // If the user deliberately scrolls during the grace window, stop snapping.
    // wheel/touch/keydown are reliable user-intent signals (scroll events
    // themselves fire for our own self-scrolls too, so we can't use those).
    messagesEl.addEventListener("wheel", cancelPostReplaySticky, { passive: true });
    messagesEl.addEventListener("touchstart", cancelPostReplaySticky, { passive: true });
    messagesEl.addEventListener("keydown", cancelPostReplaySticky);

    postReplayStickyTimer = setTimeout(function () {
      cancelPostReplaySticky();
    }, graceMs);

    function cancelPostReplaySticky() {
      cancelled = true;
      if (postReplayStickyTimer) { clearTimeout(postReplayStickyTimer); postReplayStickyTimer = null; }
      if (postReplayObserver) {
        try { postReplayObserver.disconnect(); } catch (e) {}
        postReplayObserver = null;
      }
      messagesEl.removeEventListener("wheel", cancelPostReplaySticky);
      messagesEl.removeEventListener("touchstart", cancelPostReplaySticky);
      messagesEl.removeEventListener("keydown", cancelPostReplaySticky);
    }

    // Expose the cancel so resetClientState / session switch can kill it.
    stickToBottomDuringReplayGrace._cancel = cancelPostReplaySticky;
  }

  function cancelReplayGrace() {
    if (stickToBottomDuringReplayGrace._cancel) {
      stickToBottomDuringReplayGrace._cancel();
      stickToBottomDuringReplayGrace._cancel = null;
    }
  }
  ...
  // --- DOM: Messages ---
  function addUserMessage(text, images, pastes, fromUserId, fromUserName, ts) {
    ...
    if (ts) {
      timeSpan.textContent = _formatMsgTime(ts);
    ...
    var timeStr = ts ? _formatClockTime(ts) : "";
    actions.innerHTML =
  ...
  function ensureAssistantBlock() {
    if (!currentMsgEl) {
      ...
      var timeSpan = document.createElement("span");
      timeSpan.className = "dm-bubble-time";
      if (pendingAssistantTs) {
        timeSpan.textContent = _formatMsgTime(pendingAssistantTs);
        timeSpan.dataset.ts = pendingAssistantTs;
      }
  ...
  function appendDelta(text) {
    ...
    // Fast-path for history replay: accumulate text directly without
    // frame-by-frame animation. Markdown rendering is deferred to
    // finalizeAssistantBlock() which runs once per completed turn.
    if (replayingHistory) {
  ...
  function showSuggestionChips(suggestion) {
    if (!suggestion || processing || replayingHistory) return;
  ...
  function hideSuggestionChips() {
    if (suggestionChipsEl.classList.contains("hidden")) return;
    suggestionChipsEl.innerHTML = "";
    suggestionChipsEl.classList.add("hidden");
    messagesEl.style.paddingBottom = "";
  }

  // --- AI session name suggestion (header banner) ---
  function showNameSuggestion(name, sessionId) {
    if (!headerNameSuggestionEl || !name) return;
    headerNameSuggestionEl.innerHTML = "";

    var textSpan = document.createElement("span");
    textSpan.className = "name-suggestion-text";
    textSpan.textContent = name;

    var acceptBtn = document.createElement("button");
    acceptBtn.className = "name-suggestion-accept";
    acceptBtn.innerHTML = iconHtml("check");
    acceptBtn.title = "Accept suggested name";
    acceptBtn.addEventListener("click", function () {
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "accept_session_name", id: sessionId, title: name }));
    ...
    dismissBtn.className = "name-suggestion-dismiss";
    ...
    headerNameSuggestionEl.appendChild(textSpan);
    headerNameSuggestionEl.appendChild(acceptBtn);
    headerNameSuggestionEl.appendChild(dismissBtn);
    headerNameSuggestionEl.classList.remove("hidden");
    refreshIcons(headerNameSuggestionEl);
  }

  function hideNameSuggestion() {
    if (!headerNameSuggestionEl) return;
    headerNameSuggestionEl.innerHTML = "";
    headerNameSuggestionEl.classList.add("hidden");
  }
  ...
  function resetClientState() {
    ...
    prependAnchor = null;
    loadingMore = false;
    if (historyRenderAbort) { historyRenderAbort.aborted = true; historyRenderAbort = null; }
    replayBuffer = null;
    // Drop any queued live messages for the session we just left
    prependLiveBuffer = null;
    processingHistoryChunk = false;
    // Clear the history-replay animation-suppression class if a render was aborted.
    messagesEl.classList.remove("history-replay");
    // Cancel any post-replay bottom-sticky grace window from a prior session.
    cancelReplayGrace();
    isUserScrolledUp = false;
    scrollPolicy = "auto";
    newMsgBtn.classList.add("hidden");
  ...
  function processMessage(msg) {
      // Batch-buffer history items: while replaying, accumulate messages and
      // process them all in one synchronous pass when history_done arrives.
      // This prevents intermediate browser repaints so the user sees the
      // conversation appear fully rendered at the bottom, not building top-down.
      if (replayBuffer !== null) {
        if (msg.type === "history_done") {
          // Drain the entire buffer in one synchronous batch
          var buf = replayBuffer;
          replayBuffer = null;
          for (var bi = 0; bi < buf.length; bi++) {
            try {
              processMessage(buf[bi]);
            } catch (e) {
              console.error("[replayBuffer] Error draining item " + bi + " type=" + (buf[bi] && buf[bi].type) + ":", e);
            }
          }
          // Fall through to process history_done itself
        } else {
      ...
      // Queue live WS messages arriving during a chunked history-prepend render.
      // The prepend resets the shared rendering state (currentMsgEl, currentFullText,
      // prependAnchor, etc.) to "replay mode" and restores it in finishPrepend.
      // If a live delta/assistant_ts/tool event slips in between chunks it would
      // contaminate that state and the user's message would render into the wrong
      // DOM slot (or get lost entirely).  Queue them and drain in finishPrepend.
      // processingHistoryChunk is true ONLY during the inner for-loop of a chunk,
      // so history items passed via prependOlderHistory → processMessage() pass
      // through normally.
      if (prependLiveBuffer !== null && !processingHistoryChunk) {
        prependLiveBuffer.push(msg);
        return;
      }
      ...
      switch (msg.type) {
        case "history_meta":
          ...
          replayBuffer = []; // start buffering — drain on history_done
          // Suppress tool-widget pulse/spin animations during the initial
          // batch drain.  Cleared in history_done once everything is rendered
          // and markAllToolsDone() has settled state.
          messagesEl.classList.add("history-replay");
          updateHistorySentinel();
          break;

        case "history_batch":
          // Server sends all history items in one message — process synchronously
          if (msg.items) {
            for (var hbi = 0; hbi < msg.items.length; hbi++) {
              try {
                processMessage(msg.items[hbi]);
              } catch (e) {
                console.error("[history_batch] Error processing item " + hbi + " type=" + (msg.items[hbi] && msg.items[hbi].type) + ":", e);
              }
            }
          }
          break;
        ...
        case "history_done":
          ...
          // Drop the animation-suppression class now that drain + markAllToolsDone
          // have finished.  Any live (non-historical) tool created after this
          // point will animate normally.
          messagesEl.classList.remove("history-replay");
          stopUrgentBlink();
          ...
          scrollToBottom();
          // Async content (mermaid/images/highlight/icons) reflows over the
          // next ~second and would otherwise push content in below a now-stale
          // scrollTop, leaving the user mid-conversation.  Keep snapping to
          // bottom during a grace window so they land where they expect.
          stickToBottomDuringReplayGrace(1200);
          msgNav.indexRebuild();
          updateContextPanel();
          ...
          // NOTE: We deliberately do NOT auto-fetch older history here.
          // Older messages load on-demand when the user scrolls up and the
          // .history-sentinel enters the viewport (via IntersectionObserver).
          // This keeps session switches fast and prevents the "earthquake"
          // effect of the page backfilling for minutes after opening.
          break;
        ...
        case "user_message":
          ...
          if (msg.planContent) {
            ...
            addUserMessage("Execute the following plan. Do NOT re-enter plan mode — just implement it step by step.", msg.images || null, msg.pastes || null, msg.from, msg.fromName, msg.ts);
          } else {
            addUserMessage(msg.text, msg.images || null, msg.pastes || null, msg.from, msg.fromName, msg.ts);
          }
          break;
        ...
        case "session_name_suggestion":
          if (msg.id === activeSessionId) {
            showNameSuggestion(msg.suggestedName, msg.id);
          }
          break;

        case "session_renamed":
          if (msg.id === activeSessionId) {
            headerTitleEl.textContent = msg.title;
            hideNameSuggestion();
          }
          break;
  ...
  // --- Progressive history loading ---
  // Older history is NEVER loaded automatically.  The user must click the
  // "Load earlier messages" button at the top of the conversation to pull
  // the next chunk of turns.  No scroll-based auto-load, no cascade, no
  // observer — on purpose.  This keeps session switches fast and prevents
  // the page from redrawing long-buried context the user didn't ask for.
  ...
  function prependOlderHistory(items, meta) {
    // --- Phase 1: Save current rendering state ---
    ...
    // Enable replay fast-path so text deltas accumulate directly into
    // currentFullText without per-frame DOM writes — much faster for large
    // stage-2 batches.  flushStreamBuffer() inside finalizeAssistantBlock
    // renders each completed turn's accumulated text in one pass.
    replayingHistory = true;

    // --- Phase 2: Chunked async rendering ---
    // Process items in batches, yielding to the browser between each so the
    // UI stays responsive (no multi-second freeze on large histories).  While
    // yielding, we queue any live WS messages in prependLiveBuffer so they
    // don't corrupt the prepend-render state (which has currentMsgEl etc. set
    // to "replay" values that only get restored in finishPrepend).
    var CHUNK_SIZE = 100;
    var idx = 0;
    var abort = { aborted: false };
    historyRenderAbort = abort;
    prependLiveBuffer = [];

    // Tag the scroller so CSS can kill all tool/widget animations while
    // the chunked render is in-flight.  Without this, every replayed tool
    // widget pulses for the brief window between its creation and the
    // tool_result event (later in the stream) that marks it done.
    messagesEl.classList.add("history-replay");

    function processChunk() {
      // Abort guard: session switched or state was cleared while we were yielding
      if (abort.aborted) return;

      var end = Math.min(idx + CHUNK_SIZE, items.length);
      processingHistoryChunk = true;
      ...
      if (idx < items.length) {
        // Yield to browser — lets it paint, handle input, then resume.
        // Browser scroll anchoring is disabled on #messages via CSS, and we
        // do the scroll-position fix once at the end (not per-chunk, which
        // was causing visible shuddering).
        setTimeout(processChunk, 0);
      } else {
        // All items rendered — run post-processing
        finishPrepend();
      }
    }

    // --- Phase 3: Post-processing (runs once after all chunks complete) ---
    function finishPrepend() {
      if (abort.aborted) return;
      ...
      // Any tool created during replay whose tool_result wasn't in this
      // batch (truncated subagent logs, etc.) would otherwise stay "running"
      // forever.  Mark them all done so the UI reflects historical state.
      try { markAllToolsDone(); } catch (e) { /* tools module may not be loaded yet */ }

      // Drop the animation-suppression class now that the batch is settled.
      messagesEl.classList.remove("history-replay");

      // Clear prepend mode
      prependAnchor = null;

      // Restore saved state BEFORE draining the live buffer so any queued
      // deltas/tool events operate on the correct current-turn context.
      ...
      // Final scroll fix: content inserted above anchorEl pushed it down;
      // scroll back to keep the viewport visually stable.
      if (anchorEl) {
      ...
      if (meta.hasMore) {
        ...
        // NOTE: we deliberately do NOT auto-chain another page load here.
        // The user must scroll up to see older messages, which triggers the
        // IntersectionObserver on the sentinel for one more page at a time.
      } else {
        updateHistorySentinel();
      }
      ...
      // Drain any live WS messages queued during the chunked render.  State
      // has been restored above, so these process as if they'd arrived after
      // the prepend — the correct semantics.
      var queued = prependLiveBuffer;
      prependLiveBuffer = null;
      if (queued && queued.length) {
        for (var q = 0; q < queued.length; q++) {
          try {
            processMessage(queued[q]);
          } catch (e) {
            console.error("[prependLiveBuffer] drain error on type=" + (queued[q] && queued[q].type) + ":", e);
          }
        }
      }
    }

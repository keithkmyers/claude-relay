  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "dm_favorites_updated":
          // Remove unfavorited users from conversations list so they disappear from strip
          if (cachedDmFavorites && msg.dmFavorites) {
            var removed = cachedDmFavorites.filter(function (id) {
              return msg.dmFavorites.indexOf(id) === -1;
            });
            for (var ri = 0; ri < removed.length; ri++) {
              var idx = cachedDmConversations.indexOf(removed[ri]);
              if (idx !== -1) cachedDmConversations.splice(idx, 1);
            }
          }

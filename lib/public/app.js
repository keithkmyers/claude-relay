...
import { initSidebar, renderSessionList, handleSearchResults, updateSessionPresence, updatePageTitle, getActiveSearchQuery, buildSearchTimeline, removeSearchTimeline, populateCliSessionList, renderIconStrip, initIconStrip, getEmojiCategories } from './modules/sidebar.js';
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "client_count":
          ...
          if (countEl) {
            if (msg.users && msg.users.length > 1) {
              countEl.innerHTML = "";
              for (var cui = 0; cui < msg.users.length; cui++) {
                var cu = msg.users[cui];
                ...
                cuImg.title = cu.displayName + " (@" + cu.username + ")";

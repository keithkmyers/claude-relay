...
// --- DOM refs ---
  ...
  function createCursorElement(userId, displayName, color) {
    ...
    wrapper.style.transition = "left 0.1s ease-out, top 0.1s ease-out";
    ...
    label.style.cssText = "position:absolute;left:14px;top:14px;background:" + color +
      ";color:#fff;font-size:11px;font-weight:500;padding:1px 6px;border-radius:3px;" +
      "white-space:nowrap;line-height:16px;font-family:inherit;pointer-events:none;";
  ...
  function handleRemoteCursorMove(msg) {
    ...
    if (!entry) {
      ...
      var el = createCursorElement(userId, msg.displayName, color);
  ...
  // Clean up remote cursors when switching sessions

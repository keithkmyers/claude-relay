...
export function clearScheduleDelay() {
  ...
  if (btn) btn.classList.remove("schedule-active");
}
...
export function initInput(_ctx) {
  ...
  // Schedule button — timer picker popover
  var scheduleBtn = document.getElementById("schedule-btn");
  var schedulePopover = null;

  function closeSchedulePopover() {
    if (schedulePopover) {
      schedulePopover.remove();
  ...
  function setScheduleDelay(ms) {
    ...
    if (btn) {
      if (ms > 0) {
        btn.classList.add("schedule-active");
        btn.title = "Scheduled: " + formatDelayLabel(ms) + " (click to clear)";
      } else {
        btn.classList.remove("schedule-active");
  ...
  function showSchedulePopover() {
    ...
    pop.className = "msg-schedule-popover";
    ...
    // Position relative to schedule button
    var wrap = scheduleBtn.closest("#attach-wrap");
    if (wrap) {
      wrap.style.position = "relative";
      wrap.appendChild(pop);
    } else {
      scheduleBtn.parentNode.appendChild(pop);
    }
    schedulePopover = pop;
    ...
    setTimeout(function () {
      function outsideClick(ev) {
        if (!pop.contains(ev.target) && ev.target !== scheduleBtn) {
          closeSchedulePopover();

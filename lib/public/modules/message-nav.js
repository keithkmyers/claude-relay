...
var _SCROLL_THRESHOLD = 150;   // visual "at bottom" indicator on end button
var _BREAK_THRESHOLD = 80;     // how far from bottom to break auto-follow
var _REENGAGE_THRESHOLD = 15;  // how close to bottom to re-engage auto-follow
var _REENGAGE_GRACE_MS = 200;  // cooldown after breaking before re-engage allowed
var _lastBreakTime = 0;        // timestamp of last scroll-lock break
...
function doUpdateRail() {
  ...
  // Position label — offset from the end (stable as history lazy-loads).
  // At the tail: empty (you can see you're at the end).
  // Going up: -1, -2, -3 etc.
  var offsetFromEnd = count - 1 - cur;
  if (count > 0 && cur >= 0 && offsetFromEnd > 0) {
    _posLabel.textContent = "\u2212" + offsetFromEnd;  // minus sign + number
  } else {
    _posLabel.textContent = "";
  }
...
export function initMessageNav(api) {
  ...
  _api.messagesEl.addEventListener("scroll", function () {
    if (_streaming) {
      ...
      if (_scrollPolicy === "auto" && distFromBottom > _BREAK_THRESHOLD) {
        // User scrolled up during auto-follow — break the lock.
        // Uses a smaller threshold (80px) so it's easier to escape.
        _scrollPolicy = "pinned";
        _lastBreakTime = Date.now();
        if (_api.setScrollPolicy) _api.setScrollPolicy("pinned");
      } else if (_scrollPolicy === "pinned" && distFromBottom <= _REENGAGE_THRESHOLD) {
        // User scrolled all the way back to the bottom — re-engage.
        // Uses a tight threshold (15px) so casual proximity doesn't
        // snap you back.  Also enforces a grace period after breaking
        // so momentary bounces don't immediately re-engage.
        if (Date.now() - _lastBreakTime >= _REENGAGE_GRACE_MS) {
          _scrollPolicy = "auto";
          if (_api.setScrollPolicy) _api.setScrollPolicy("auto");
        }
      }
      // Update at-bottom visual state on the end button (uses the
      // wider visual threshold, independent of scroll-lock policy)

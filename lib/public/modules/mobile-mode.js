/**
 * Docked Nav & Mobile Mode — the message navigation rail is always docked
 * as a "sidecar" beside the input box.  This avoids fragile absolute
 * positioning that breaks in wide/channel view and when side panels open.
 *
 * On mobile, `.mobile-typing` hides the sidecar when the input is focused
 * so the keyboard doesn't fight for space.
 *
 * `.nav-docked` is always set on #app.  The rail lives inside
 * #input-wrapper at all times.
 */
 ...
 * Always true — the sidecar is now the permanent home for the nav rail.
 */

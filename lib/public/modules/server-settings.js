...
export function initServerSettings(appCtx) {
  ...
  // Windmills toggles — each persists to config.windmills.<key> via the generic IPC  // windmills
  var wmAdoptToggle = document.getElementById("settings-wm-adopt");  // windmills
  if (wmAdoptToggle) {  // windmills
    wmAdoptToggle.addEventListener("change", function () {  // windmills
      var ws = ctx.ws;  // windmills
      if (ws && ws.readyState === 1) {  // windmills
        ws.send(JSON.stringify({ type: "set_windmills_setting", key: "adoptCliSessions", value: this.checked }));  // windmills
      }  // windmills
    });  // windmills
  }  // windmills
  var wmFilterConfigToggle = document.getElementById("settings-wm-filter-config");  // windmills
  if (wmFilterConfigToggle) {  // windmills
    wmFilterConfigToggle.addEventListener("change", function () {  // windmills
      var ws = ctx.ws;  // windmills
      if (ws && ws.readyState === 1) {  // windmills
        ws.send(JSON.stringify({ type: "set_windmills_setting", key: "filterConfigSessions", value: this.checked }));  // windmills
      }  // windmills
    });  // windmills
  }  // windmills
  var wmAutoToggle = document.getElementById("settings-wm-auto");  // windmills
  if (wmAutoToggle) {  // windmills
    wmAutoToggle.addEventListener("change", function () {  // windmills
      var ws = ctx.ws;  // windmills
      if (ws && ws.readyState === 1) {  // windmills
        ws.send(JSON.stringify({ type: "set_windmills_setting", key: "allowAutoMode", value: this.checked }));  // windmills
      }  // windmills
    });  // windmills
  }  // windmills
  var wmModelPickerToggle = document.getElementById("settings-wm-model-picker");  // windmills
  if (wmModelPickerToggle) {  // windmills
    wmModelPickerToggle.addEventListener("change", function () {  // windmills
      var ws = ctx.ws;  // windmills
      if (ws && ws.readyState === 1) {  // windmills
        ws.send(JSON.stringify({ type: "set_windmills_setting", key: "allowClaudeGuiModelPicker", value: this.checked }));  // windmills
      }  // windmills
    });  // windmills
  }  // windmills
...
export function updateDaemonConfig(config) {
  ...
  // Windmills toggles (default ON: absent or !== false means ON)  // windmills
  var wm = config.windmills || {};  // windmills
  var wmAdopt = document.getElementById("settings-wm-adopt");  // windmills
  if (wmAdopt) wmAdopt.checked = wm.adoptCliSessions !== false;  // windmills
  var wmFilterConfig = document.getElementById("settings-wm-filter-config");  // windmills
  if (wmFilterConfig) wmFilterConfig.checked = wm.filterConfigSessions !== false;  // windmills
  var wmAuto = document.getElementById("settings-wm-auto");  // windmills
  if (wmAuto) wmAuto.checked = wm.allowAutoMode !== false;  // windmills
  var wmModelPicker = document.getElementById("settings-wm-model-picker");  // windmills
  if (wmModelPicker) wmModelPicker.checked = wm.allowClaudeGuiModelPicker !== false;  // windmills
...
export function handleWindmillsChanged(msg) {  // windmills
  var wm = msg.windmills || {};  // windmills
  var wmAdopt = document.getElementById("settings-wm-adopt");  // windmills
  if (wmAdopt) wmAdopt.checked = wm.adoptCliSessions !== false;  // windmills
  var wmFilterConfig = document.getElementById("settings-wm-filter-config");  // windmills
  if (wmFilterConfig) wmFilterConfig.checked = wm.filterConfigSessions !== false;  // windmills

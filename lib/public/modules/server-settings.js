...
function switchSection(sectionName) {
  ...
  if (sectionName === "ai-features") loadAIFeaturesSettings();
...
// --- AI Features (Windmills) ---
var _aiSettingsLoaded = false;

function loadAIFeaturesSettings() {
  if (_aiSettingsLoaded) return;
  _aiSettingsLoaded = true;
  fetch("/api/settings/auto-naming").then(function (r) { return r.json(); }).then(function (data) {
    var radios = document.querySelectorAll('#settings-auto-naming input[name="auto-naming"]');
    var val = data.autoNaming || "off";
  ...
  for (var i = 0; i < radios.length; i++) {
    radios[i].addEventListener("change", function () {
      ...
      fetch("/api/settings/auto-naming", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ value: value }),
      }).then(function (r) { return r.json(); }).then(function (data) {
        if (data.ok) {
          var labels = { off: "Off", suggest: "Suggest", auto: "Auto" };
          showToast("Session naming: " + (labels[data.autoNaming] || data.autoNaming));
        }
      }).catch(function () {});
    });
  }

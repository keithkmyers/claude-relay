...
function toggleDmUserPicker(anchorEl) {
  ...
  // Re-addable built-in mates section
  var builtinSection = document.createElement("div");
  builtinSection.className = "dm-user-picker-builtin-section";
  builtinSection.style.display = "none";
  picker.appendChild(builtinSection);

  // Request available built-in mates
  if (ctx.sendWs) {
    ctx.sendWs({ type: "mate_list_available_builtins" });
  }

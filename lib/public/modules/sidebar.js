...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  var currentStatus = (sessionData && sessionData.status) || null;
  var isDone = currentStatus === "done";
  var statusItem = document.createElement("button");
  statusItem.className = "session-ctx-item";
  statusItem.innerHTML = iconHtml(isDone ? "circle-x" : "circle-check") + " <span>" + (isDone ? "Clear Status" : "Mark Done") + "</span>";
  statusItem.addEventListener("click", function (e) {
    e.stopPropagation();
    closeSessionCtxMenu();
    var newStatus = isDone ? null : "done";
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({ type: "set_session_status", sessionId: sessionId, status: newStatus }));
    }
  });
  ...
  restartItem.innerHTML = iconHtml("refresh-cw") + " <span>Restart Claude</span>";
  restartItem.addEventListener("click", function (e) {
    e.stopPropagation();
    closeSessionCtxMenu();
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({ type: "restart_session", sessionId: sessionId }));
    }
  });
...
function renderSessionItem(s) {
  ...
  var isMatch = searchMatchIds !== null && searchMatchIds.has(s.id);
  var dimmed = searchMatchIds !== null && !isMatch;
  el.className = "session-item" + (s.active ? " active" : "") + (isMatch ? " search-match" : "") + (dimmed ? " search-dimmed" : "") + (s.status === "done" ? " status-done" : "");
  el.dataset.sessionId = s.id;
  ...
  if (s.status === "done") {
    textHtml += '<span class="session-status-icon done" title="Done">' + iconHtml("circle-check") + '</span>';
  }

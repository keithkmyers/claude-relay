...
export function toolSummary(name, input) {
  ...
  switch (name) {
    ...
    case "Bash": var cmd = input.command || ""; return cmd.length > 2000 ? cmd.substring(0, 2000) + "..." : cmd;
...
export function updateToolExecuting(id, name, input) {
  ...
  // Bash: add tooltip so full command is visible on hover when collapsed
  if (name === "Bash" && input && input.command) {
    var ttCmd = input.command;
    descEl.title = ttCmd.length > 2000 ? ttCmd.substring(0, 2000) + "..." : ttCmd;
  }
...
export function updateToolResult(id, content, isError, images) {
  ...
  // Bash: prepend a command block showing the full command, visually separated from output
  if (tool.name === "Bash" && tool.input && tool.input.command) {
    var rawCmd = tool.input.command;
    var cmdBlock = document.createElement("div");
    cmdBlock.className = "tool-command-block";
    cmdBlock.textContent = rawCmd.length > 2000 ? rawCmd.substring(0, 2000) + "..." : rawCmd;
    resultBlock.insertBefore(cmdBlock, resultBlock.firstChild);
  }

...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images) {
    ...
    // If session was created with acceptEdits flag (e.g. plan clear+auto-accept), set mode
    if (session.acceptEditsAfterStart) {
      delete session.acceptEditsAfterStart;
      session.queryInstance.setPermissionMode("acceptEdits").catch(function(e) {
        send({ type: "error", text: "Failed to set acceptEdits mode: " + (e.message || e) });
      });
    }

...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images) {
    ...
    // Apply stored permission mode (from config chip or plan approval)
    ...
    if (modeToApply && modeToApply !== "default") {
      session.queryInstance.setPermissionMode(modeToApply).catch(function(e) {
        send({ type: "error", text: "Failed to set permission mode (" + modeToApply + "): " + (e.message || e) });
      });
    }
  ...
  async function setModel(session, model) {
    ...
    try {
      ...
      send({ type: "config_state", model: sm.currentModel, mode: sm.currentPermissionMode || "default", effort: sm.currentEffort || "high" });
    } catch (e) {
  ...
  async function setPermissionMode(session, mode) {
    if (!session.queryInstance) {
      ...
      send({ type: "config_state", model: sm.currentModel || "", mode: sm.currentPermissionMode, effort: sm.currentEffort || "high" });
      return;
    }
    try {

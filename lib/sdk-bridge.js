...
var modelDefs = require("./model-defs");
...
function createSDKBridge(opts) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.type === "system" && parsed.subtype === "init") {
      ...
      if (parsed.model) {
        // The SDK reports back resolved model names (e.g. "claude-opus-4-6[1m]")
        // which differ from the short names the user selected (e.g. "opus").
        // Use the alias table to avoid overwriting the user's selection.
        var reportedModel = sm._savedDefaultModel || parsed.model;
        if (!sm.currentModel || !modelDefs.resolveModelAlias(sm.currentModel, reportedModel)) {
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (evt.type === "content_block_delta" && evt.delta) {
        ...
        if (evt.delta.type === "text_delta" && typeof evt.delta.text === "string") {
          // Emit a timestamp marker before the first text delta so the client
          // (and history replay) knows when the assistant response started.
          if (!session.streamedText) {
            sendAndRecord(session, { type: "assistant_ts", ts: Date.now() });
          }
      ...
      // System-initiated AI session naming: trigger after first turn
      console.log("[naming-check] session=" + session.localId +
        " hasFirstMsg=" + !!session._firstMessageText +
        " firstMsgLen=" + (session._firstMessageText || "").length +
        " aiNamingDone=" + !!session.aiNamingDone +
        " hasCallback=" + (typeof opts.onSessionNamingTrigger === "function"));
      if (session._firstMessageText && !session.aiNamingDone) {
        session.aiNamingDone = true;
        sm.saveSessionFile(session);
        if (typeof opts.onSessionNamingTrigger === "function") {
          try {
            console.log("[naming-check] calling onSessionNamingTrigger for session=" + session.localId);
            opts.onSessionNamingTrigger(session);
          } catch (e) {
            console.error("[naming-check] onSessionNamingTrigger threw:", e.message, e.stack);
          }
        }
      }
  ...
  async function startQueryViaWorker(session, text, images, linuxUser) {
    ...
    if (sm.currentModel) queryOptions.model = modelDefs.resolveModelForSdk(sm.currentModel);
  ...
  async function warmupViaWorker(linuxUser) {
    ...
    worker.onMessage(function(msg) {
      if (msg.type === "warmup_done" && !warmupDone) {
        ...
        if (result.model) {
          var workerReported = sm._savedDefaultModel || result.model;
          if (!sm.currentModel || !modelDefs.resolveModelAlias(sm.currentModel, workerReported)) {
            sm.currentModel = workerReported;
          }
        }
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    if (sm.currentModel) {
      queryOptions.model = modelDefs.resolveModelForSdk(sm.currentModel);
    }
    ...
    try {
      ...
      for await (var msg of stream) {
        if (msg.type === "system" && msg.subtype === "init") {
          ...
          if (msg.model) {
            // Use alias table so the SDK's resolved name doesn't overwrite the user's selection
            if (!sm.currentModel || !modelDefs.resolveModelAlias(sm.currentModel, msg.model)) {
              sm.currentModel = msg.model;
            }
          }
          ...
          try {
            ...
            sm.availableModels = modelDefs.ensureCustomModels(models || []);
          } catch (e) {}
          send({ type: "model_info", model: sm.currentModel || "", models: sm.availableModels || [] });
          ac.abort();
          break;
  ...
  async function setModel(session, model) {
    var sdkModel = modelDefs.resolveModelForSdk(model);
    if (session.worker) {
      session.worker.send({ type: "set_model", model: sdkModel });
      return;
    }
    if (!session.queryInstance) {

...
function createSDKBridge(opts) {
   ...
   */
  function findConflictingClaude(session) {  // windmills: conflict-detection — same-session only
    // windmills: the ONLY real conflict is the SAME session running twice. Never flag the user's
    // other concurrent agents in this project (running many `claude` per project is normal + safe).
    var _wmSid = session && (session.cliSessionId || session.sdkSessionId || session.sessionId || session.id);
    try {
      ...
      for (var i = 1; i < lines.length; i++) { // skip header
        ...
        if (/\/claude(\s|$)/.test(cmd) || /^claude(\s|$)/.test(cmd)) {
          if (!_wmSid || cmd.indexOf(_wmSid) === -1) continue;  // windmills: skip unless it's the SAME session id
    ...
    try {
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          ...
          // windmills: conflict-detection — match EXACT "code 1" (was indexOf, which also matched
          // "code 127"/1xx → every binary-launch failure was mislabeled a process conflict).
          var isExitCode1 = err.exitCode === 1 || (err.message && /exited with code 1(\D|$)/.test(err.message));
          var conflicts = isExitCode1 ? findConflictingClaude(session) : [];  // windmills: pass session for same-session-only match

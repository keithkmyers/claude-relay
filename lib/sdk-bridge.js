...
function createSDKBridge(opts) {
   ...
   */
  function findConflictingClaude(session) {  // windmill: conflict-detection — same-session only
    // windmill: the ONLY real conflict is the SAME session running twice. Never flag the user's
    ...
    try {
      ...
      for (var i = 1; i < lines.length; i++) { // skip header
        ...
        if (/\/claude(\s|$)/.test(cmd) || /^claude(\s|$)/.test(cmd)) {
          if (!_wmSid || cmd.indexOf(_wmSid) === -1) continue;  // windmill: skip unless it's the SAME session id
    ...
    try {
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          ...
          // windmill: conflict-detection — match EXACT "code 1" (was indexOf, which also matched
          ...
          var conflicts = isExitCode1 ? findConflictingClaude(session) : [];  // windmill: pass session for same-session-only match

...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    // --- Server logo customization (server-wide, not per-user) ---
    var serverLogoPath = path.join(CONFIG_DIR, "server-logo.json");

    if (req.method === "GET" && fullUrl === "/api/server-logo") {
      try {
        var logoData = JSON.parse(fs.readFileSync(serverLogoPath, "utf8"));
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(logoData));
      } catch (e) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end('{"type":"default"}');
      }
      return;
    }

    if (req.method === "PUT" && fullUrl === "/api/server-logo") {
      var body = "";
      req.on("data", function (chunk) { body += chunk; });
      req.on("end", function () {
        try {
          var data = JSON.parse(body);
          var safe = {
            type: String(data.type || "default").substring(0, 20),
            url: String(data.url || "").substring(0, 500),
            emoji: String(data.emoji || "").substring(0, 8),
            color: String(data.color || "").substring(0, 20),
          };
          fs.writeFileSync(serverLogoPath + ".tmp", JSON.stringify(safe, null, 2));
          fs.renameSync(serverLogoPath + ".tmp", serverLogoPath);
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end('{"ok":true}');
        } catch (e) {
          res.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (req.method === "POST" && fullUrl === "/api/server-logo/upload") {
      var chunks = [];
      var totalSize = 0;
      var maxSize = 2 * 1024 * 1024; // 2MB
      req.on("data", function (chunk) {
      ...
      req.on("end", function () {
        ...
        var ext = ct.split("/")[1] === "jpeg" ? "jpg" : ct.split("/")[1];
        // Remove old server-logo image files
        try {
          var dirFiles = fs.readdirSync(CONFIG_DIR);
          for (var di = 0; di < dirFiles.length; di++) {
            if (dirFiles[di].startsWith("server-logo.") && dirFiles[di] !== "server-logo.json" && dirFiles[di] !== "server-logo.json.tmp") {
              fs.unlinkSync(path.join(CONFIG_DIR, dirFiles[di]));
            }
          }
        } catch (e) {}
        var logoFilePath = path.join(CONFIG_DIR, "server-logo." + ext);
        fs.writeFileSync(logoFilePath, raw);
        try { fs.chmodSync(logoFilePath, 0o644); } catch (e) {}
        var logoUrl = "/api/server-logo/image?v=" + Date.now();
        // Also update the JSON config
        var logoConfig = { type: "custom", url: logoUrl, style: "", seed: "", color: "" };
        try {
          var existing = JSON.parse(fs.readFileSync(serverLogoPath, "utf8"));
          if (existing.color) logoConfig.color = existing.color;
        } catch (e) {}
        fs.writeFileSync(serverLogoPath + ".tmp", JSON.stringify(logoConfig, null, 2));
        fs.renameSync(serverLogoPath + ".tmp", serverLogoPath);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ ok: true, url: logoUrl }));
      });
      return;
    }

    if (req.method === "GET" && fullUrl.startsWith("/api/server-logo/image")) {
      try {
        var dirFiles = fs.readdirSync(CONFIG_DIR);
        var logoFile = null;
        for (var di = 0; di < dirFiles.length; di++) {
          if (dirFiles[di].startsWith("server-logo.") && dirFiles[di] !== "server-logo.json" && dirFiles[di] !== "server-logo.json.tmp") {
            logoFile = dirFiles[di];
            break;
          }
        }
        if (logoFile) {
          var ext = logoFile.split(".").pop();
          var ctMap = { jpg: "image/jpeg", png: "image/png", gif: "image/gif", webp: "image/webp" };
          res.writeHead(200, {
            "Content-Type": ctMap[ext] || "application/octet-stream",
            "Cache-Control": "public, max-age=31536000, immutable",
          });
          res.end(fs.readFileSync(path.join(CONFIG_DIR, logoFile)));
          return;
        }
      } catch (e) {}
      res.writeHead(404, { "Content-Type": "application/json" });
    ...
    // PUT /api/settings/auto-naming
    if (req.method === "PUT" && fullUrl === "/api/settings/auto-naming") {
      var mu = getMultiUserFromReq(req);
      if (!mu) {
        // Single-user: use daemon config
        var body = "";
        req.on("data", function (chunk) { body += chunk; });
        req.on("end", function () {
          try {
            var data = JSON.parse(body);
            if (typeof opts.onSetAutoNaming === "function") {
              opts.onSetAutoNaming(data.value || "off");
            }
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ ok: true, autoNaming: data.value || "off" }));
          } catch (e) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end('{"error":"Invalid request"}');
          }
      ...
      req.on("end", function () {
        try {
          var data = JSON.parse(body);
          var result = users.setAutoNaming(mu.id, data.value || "off");
          if (result.error) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: result.error }));
            return;
          ...
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ ok: true, autoNaming: result.autoNaming }));
        } catch (e) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end('{"error":"Invalid request"}');
        }
    ...
    // GET /api/settings/auto-naming
    if (req.method === "GET" && fullUrl === "/api/settings/auto-naming") {
      var mu = getMultiUserFromReq(req);
      if (!mu) {
        var val = "off";
        if (typeof opts.onGetDaemonConfig === "function") {
          var dc = opts.onGetDaemonConfig();
          val = dc.autoNaming || "off";
        }
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ autoNaming: val }));
        return;
      }
      var val = users.getAutoNaming(mu.id);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ autoNaming: val }));
      return;
    }

...
function createSessionManager(opts) {
  ...
  function saveSessionFile(session) {
    ...
    try {
      ...
      if (session.aiNamingDone) metaObj.aiNamingDone = true;
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      if (m.status) session.status = m.status;
      if (m.aiNamingDone) session.aiNamingDone = true;
  ...
  var HISTORY_PAGE_TURNS = 50;         // complete turns to include in full load
  var HISTORY_QUICK_TURNS = 10;        // turns shown on session open (older load on scroll)
  var HISTORY_LOAD_MORE_TURNS = 10;    // turns fetched per "load more" when user scrolls up
  ...
  // Walk backwards from `beforeIndex` (default: end of history) and return
  // the index of the Nth user_message.  This guarantees we return a complete
  // turn window regardless of how many items each turn contains (thinking
  // deltas, text deltas, tool events, etc.) — pagination by turns, not by
  // raw item count.
  function findFromIndexForTurns(history, turnCount, beforeIndex) {
    var start = typeof beforeIndex === "number" ? beforeIndex - 1 : history.length - 1;
    if (start >= history.length) start = history.length - 1;
    var turnsSeen = 0;
    for (var i = start; i >= 0; i--) {
      if (history[i] && history[i].type === "user_message") {
        turnsSeen++;
        if (turnsSeen >= turnCount) return i;
      }
    }
    return 0;
  }
  ...
  function replayHistory(session, fromIndex, targetWs, transform) {
    ...
    // Quick-load only: send just the last few turns immediately so the UI
    // is interactive.  Older messages load on-demand when the user scrolls
    // up and the history sentinel enters the viewport.  We deliberately do
    // NOT auto-backfill older turns here — that was causing long-running
    // render storms on mobile ("earthquake" effect) where scroll and reads
    // were disrupted for minutes after opening a long conversation.
    var quickFrom = findFromIndexForTurns(session.history, HISTORY_QUICK_TURNS);
    // The `fromIndex` argument (used by code that explicitly wants a bigger
    // initial window, e.g. rewind restore) still takes precedence if smaller.
    if (typeof fromIndex === "number" && fromIndex < quickFrom) {
      quickFrom = fromIndex;
    }

    _send({ type: "history_meta", total: total, from: quickFrom });

    var quickItems = [];
    for (var i = quickFrom; i < total; i++) {
      quickItems.push(transform ? transform(session.history[i]) : session.history[i]);
    }
    _send({ type: "history_batch", items: quickItems });
  ...
  return {
    ...
    HISTORY_PAGE_TURNS: HISTORY_PAGE_TURNS,
    HISTORY_QUICK_TURNS: HISTORY_QUICK_TURNS,
    HISTORY_LOAD_MORE_TURNS: HISTORY_LOAD_MORE_TURNS,
    ...
    findFromIndexForTurns: findFromIndexForTurns,
    ...
    setSessionStatus: function (localId, status) {
      var session = sessions.get(localId);

...
function createSessionManager(opts) {
  ...
  loadSessions();
  // clay local patch (P3.T5): CLI-session adoption OFF by default — it floods the
  // list with every ~/.claude CLI session, and UI-deletes get re-adopted on restart.
  // Opt in with CLAY_ADOPT_CLI_SESSIONS=1 (deliberately not set in the systemd unit).
  if (process.env.CLAY_ADOPT_CLI_SESSIONS === "1") adoptOrphanedCliSessions();

...
function createSessionManager(opts) {
  ...
  var getDaemonConfig = opts.getDaemonConfig || function () { return {}; };  // windmills
  function wmConfig() { return (getDaemonConfig() || {}).windmills || {}; }  // windmills: live windmills.* settings
  ...
  function adoptOrphanedCliSessions() {
    // windmills: skip "configuration sessions" — sessions created only to run slash commands
    // (/effort, /compact, …) with no real conversation (0 assistant turns + 0 genuine prompts).
    var filterConfigSessions = wmConfig().filterConfigSessions !== false;  // default ON
    var WM_CMD_MARKERS = ["<command-name>", "<command-message>", "<command-args>", "<local-command-stdout>", "<local-command-caveat>", "<local-command-stderr>"];
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var realUserCount = 0;  // windmills: genuine prompts (not slash-command/meta/tool-result)
        ...
        if (ev.type === "user" && ev.message && ev.message.role === "user") {
          ...
          if (typeof c === "string") {
            ...
            for (var ci = 0; ci < c.length; ci++) {
              ...
              if (c[ci] && c[ci].type === "tool_result") hasToolResult = true;
            }
            msgText = parts.join("");
          }
          // windmills: count genuine prompts only (skip meta, slash-command, tool-result)
          var isCmd = false;
          for (var mi = 0; mi < WM_CMD_MARKERS.length; mi++) { if (msgText.indexOf(WM_CMD_MARKERS[mi]) !== -1) { isCmd = true; break; } }
          if (!(ev.isMeta === true) && !isCmd && !hasToolResult && msgText.trim() !== "") realUserCount++;
          if (firstUserText == null) {
            firstUserText = msgText;
      ...
      // windmills: skip configuration sessions (slash-command-only, no real conversation)
      if (filterConfigSessions && assistantCount === 0 && realUserCount === 0) continue;
  ...
  loadSessions();
  // windmills: CLI-session adoption is controlled by windmills.adoptCliSessions (default ON);
  // configuration-only sessions (slash-command-only, no real conversation) are skipped when
  // windmills.filterConfigSessions is ON (default). See adoptOrphanedCliSessions().
  if (wmConfig().adoptCliSessions !== false) adoptOrphanedCliSessions();

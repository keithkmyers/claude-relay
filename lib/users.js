...
// --- Per-user AI session naming setting ---

function getAutoNaming(userId) {
  var data = loadUsers();
  for (var i = 0; i < data.users.length; i++) {
    if (data.users[i].id === userId) {
      var val = data.users[i].autoNaming;
      if (val === "suggest" || val === "auto") return val;
      return "off";
    }
  }
  return "off";
...
function setAutoNaming(userId, value) {
  var data = loadUsers();
  var valid = ["off", "suggest", "auto"];
  var want = valid.indexOf(value) !== -1 ? value : "off";
  for (var i = 0; i < data.users.length; i++) {
    if (data.users[i].id === userId) {
      data.users[i].autoNaming = want;
      saveUsers(data);
      return { ok: true, autoNaming: want };
    }
  }
  return { error: "User not found" };
}
...
module.exports = {
  ...
  getAutoNaming: getAutoNaming,
  setAutoNaming: setAutoNaming,
